...
(function($) {
	...
	$.extend($.ZoomBox, {
		...
		defaults: {
			...
			loading_class: "zoom-loading",

			// the attribute on your thumbnails that points to the high resolution image
			// you want to use in the zoom. Eg: <img src='thumb.jpg' data-image='highres.jpg'/>
			src_attribute: "data-image",
		...
		prototype: {
			...
			position_container: function() {
				...
				function clamp(x, y) {
					...
					switch (direction) {
						case "left":
							return [Math.max(x, scroll_x + margin), top]
						case "top":
							return [left, Math.max(y, scroll_y + margin)]
						case "bottom":
							return [left, Math.min(y, scroll_y + screen_h - margin - cont_h)]
						default:
							return [Math.min(x, scroll_x + screen_w - margin - cont_w), top]
					}
				}
